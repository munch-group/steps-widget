...
def _steps(_expr, _print_steps=False, _with_labels=False):
    ...
    if sys.version_info >= (3, 11):
        raise RuntimeError(
            "steps_widget's stepper dispatches on CPython 3.9/3.10 bytecode opcode "
            "names (e.g. BINARY_ADD, CALL_FUNCTION) that Python 3.11 restructured "
            "(collapsed into BINARY_OP, PRECALL/CALL, ...). Install/run steps-widget "
            "on Python 3.9 or 3.10."
        )
